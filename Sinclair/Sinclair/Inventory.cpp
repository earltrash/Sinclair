...
bool Inventory::HandleMouseDown(Vec2 mousePos) //æÓ¬˜«« Inven ¿ßƒ° ≥ªø° ¿÷æÓæﬂ ¿Ã∞‘ √≥∏Æ∞° µ«¥¬ ∞≈¥œ±Ò 
{
    ...
    if (slot && !slot->IsEmpty() && slot->isEnabled)
    {
        ...
        CursorManager::Get().SetDraggedItem(m_itemDatabase.GetItemData(slot->item.id)); // Ω«¡¶ Item* ¿¸¥ﬁ
...
bool Inventory::HandleMouseUp(Vec2 mousePos) //±◊ ≥ı¿∫ ¿ßƒ°ø° ¥Î«— øπø‹√≥∏Æ∏¶ «ÿæﬂ «‘. ¥©∞° Ω√¿€ «ﬂ¥¬¡ˆ æÀæ∆æﬂ «“ µÌ 
{
    ...
    if (targetSlot && targetSlot->isEnabled && draggedItemData)
    {
        ...
        {
            // ∫Û ΩΩ∑‘ø° µÂ∑”
            targetSlot->SetItem(draggedItemData->m_data.id, 1); // count¥¬ CursorManagerø°º≠ ∞¸∏Æ«ÿæﬂ «‘
            targetSlot->UpdateItemBitmap(&controller, &m_itemDatabase);
            placed = true;
        }
    ...
    if (IsInBounds(mousePos))
    {
        // √¢ øµø™ ≥ªø°º≠ µÂ∑°±◊µ» æ∆¿Ã≈€¿Ã ¿÷¿∏∏È ¿Œ∫•≈‰∏Æ∑Œ π›»Ø
    ...
    if (placed)
    {
        CursorManager::Get().EndItemDrag(); // µÂ∑” º∫∞¯ Ω√ µÂ∑°±◊ ¡æ∑·
        return true;
    }
    else
    {
...
bool Inventory::HandleDropFailure(Vec2 mousePos, Item* draggedItem, DragSource source)
{
    ...
    // 2. ¥Ÿ∏• √¢ øµø™¿Ã∏È «ÿ¥Á √¢ø°º≠ √≥∏Æ«œµµ∑œ ≥—±Ë (µÂ∑°±◊ ªÛ≈¬ ¿Ø¡ˆ)
    if (isInOtherWindow)
    {
        return false; // ¥Ÿ∏• √¢ø°º≠ √≥∏Æ«œµµ∑œ ≥—±Ë
    }
...
bool Inventory::HandleMouseRight(Vec2 mousePos) //ªÁøÎ«— æ∆¿Ã≈€¿« ∆˜¿Œ≈Õ∏¶ πﬁæ∆øÕº≠ æ¯æ÷¥¬ Ωƒ¿∏∑Œ ¡¯«‡«ÿæﬂ «“µÌ. 
{
    ...
    if (slot && !slot->IsEmpty())
    {
        ...
        if (item != nullptr)
        {
            ...
            m_pendingPotionSlot = slot; //  ±‚æÔ

            UIManager::Get().ShowPotionWindow(much); //∆˜¡ˆº«µµ ∏¬√Á πˆ∑»¥Ÿ∞Ì 
        ...
        if (wear != nullptr)
        {
            ...
            UIManager::Get().OpenWindow(UIWindowType::EquipmentWindow); //»∞º∫»≠ Ω√µµ 
        ...
        for (int y = 0; y < region.gridSize.y; ++y)
        {
            for (int x = 0; x < region.gridSize.x; ++x)
            {
                ...
                if (slots.count(key))
                {
                    ...
                    // ?¨Î°Ø ?ÑÏπò???¥Îãπ Region??boundsÎ•?Í∏∞Ï??ºÎ°ú Í≥ÑÏÇ∞ (regions.bounds??InitializeRegions?êÏÑú windowPosition???∞Îùº ?¥Î? Í≤∞Ï†ï??
                    //float slotX = region.bounds.x + x * totalSlotDimension_x;
                    //float slotY = region.bounds.y + y * totalSlotDimension_y;
...
void Inventory::PackItem() //«ˆ¿Á databaseø° ¿÷¥¬ ∏µÁ Item¿ª Slotø° ≥÷æÓ¡‹ 
{
...
void Inventory::SetDatabase(unique_ptr<ItemDatabase> database) //ø‹∫Œø°º≠ ∏∏µÈ∞Ì ≥÷æÓπˆ∏±∑°/
{
